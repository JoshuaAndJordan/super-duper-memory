...
utils::waitable_container_t<account_monitor_task_result_t>
    monitorStatusResults;
...
void addKucoinAccountStream(
    kucoin_stream_list_t &, account_info_t const &, trade_type_e,
    net::io_context& ioContext, ssl::context &sslContext);
...
void exchangeResultWatcher(std::string const &pathPrefix,
                           ...
                           Stream &resultStream, bool &isRunning)
{
  auto const address =
      fmt::format("ipc://{}/{}", pathPrefix, exchangeName);
  ...
  while (isRunning) {
    ...
    std::visit([&outBuffer, &writerSocket](auto && data) mutable {
...
std::optional<account_scheduled_task_t> get_scheduled_task(zmq::socket_t& socket)
{
...
void binanceAccountMonitor(std::string const & path,
                           zmq::context_t& msgContext,
                           ...
                           bool& isRunning)
{
  ...
  while (isRunning) {
    ...
    if (optTask->operation == task_operation_e::add) {
      addBinanceAccountStream(binanceStreams, accountInfo, ioContext, sslContext);
                          ...
                          bool& isRunning)
{
  ...
  while (isRunning) {
    ...
    if (optTask->operation == task_operation_e::add) {
      addKucoinAccountStream(streams, accountInfo, optTask->tradeType,ioContext, sslContext);
    } else if (optTask->operation == task_operation_e::remove) {
                        ...
                        bool& isRunning)
{
  ...
  while (isRunning) {
    ...
    else result.state = task_state_e::stopped;
    monitorStatusResults.append(result);
  }
  receiverSocket.close();
}
