...
#pragma warning(disable : 4996)
  ...
  void okex_stream_t::perform_ssl_handshake(resolver_results_t::endpoint_type const &ep) {
    auto const host =
        fmt::format("{}:{}", ws_api_host, ep.port());
  ...
  void okex_stream_t::perform_user_login() {
    ...
    auto const sign =
        utils::base64Encode(utils::hmac256Encode(
            concatenated_data, m_accountInfo->secretKey
        ));
  ...
  void okex_stream_t::interpret_generic_messages() {
    char const *const buffer_cstr = static_cast<char const *>(m_buffer->cdata().data());
    std::string_view const buffer(buffer_cstr, m_buffer->size());
  ...
  void okex_stream_t::process_orders_pushed_data(json::array_t const &data_list) {
    for (auto const &data_json: data_list) {
      ...
      order_info.lastFilledFee =
          data_object.at("fillFee").get<json::string_t>();
      ...
      order_info.amendErrorMessage =
          data_object.at("msg").get<json::string_t>();
  ...
  void removeOkexAccountStream(std::vector<std::shared_ptr<okex_stream_t>> &streams,
                               account_info_t const &accountInfo) {
