...
namespace jordan {
  ...
  queue_monitoring_task(account_scheduled_task_t const &task) {
    ...
    do {
      iter = std::find_if(monitoredTaskResults.begin(), monitoredTaskResults.end(),
  ...
  void write_data_to_wire(zmq::context_t& msgContext,
  ...
  void launch_account_monitor_sender(bool &isRunning) {
    ...
    std::thread monitorResultThread {
      [&msgContext, &isRunning] {
