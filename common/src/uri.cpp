...
std::string uri_t::protocol() const { return m_protocol; }

std::string uri_t::path() const { return m_path; }

std::string uri_t::host() const { return m_host; }
...
void uri_t::parse(std::string const &url_s) {
  ...
  std::transform(prot_i, path_i, std::back_inserter(m_host),
                 [](int const c) { return std::tolower(c); });
