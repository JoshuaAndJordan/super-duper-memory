...
server_t::server_t(net::io_context &context, command_line_interface_t &&args)
    : m_ioContext(context),
      m_endpoint(net::ip::make_address(args.ip_address), args.port),
...
void server_t::acceptConnections() {
  m_acceptor.async_accept(
      ...
      [this](beast::error_code const ec, net::ip::tcp::socket socket) {
        return onConnectionAccepted(ec, std::move(socket));
      });
}
